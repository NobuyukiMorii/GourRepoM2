...
class ApiMoviesController extends AppController {
	...
	*GET送信
	*/
   	public function returnMoviesJsonGET(){ 
		//ポストされた場合
		if(!empty($this->request->query['areaname'])){

			//送信されてきたデータをviewに渡す
			$areaname = $this->request->query['areaname'];
	        ...
	        $UserName = $this->UserProfile->find('all',array(
	        	'conditions'=>
	        		array( '`UserProfile`.`name` LIKE ' => '%'.$this->request->query['areaname'].'%'
	        			),
	        	'fields' =>array('user_id','name')
	        ));
	        ...
	        if(!empty($UserName)){
	        	...
	        	$conditions = array(
					'OR' =>
					array(	
						'`Movie`.`title` LIKE '			     		=> '%'.$this->request->query['areaname'].'%',
						'`Movie`.`description` LIKE '	     		=> '%'.$this->request->query['areaname'].'%',
						'`Restaurant`.`name` LIKE '          		=> '%'.$this->request->query['areaname'].'%',
						'`Restaurant`.`access_line` LIKE '   		=> '%'.$this->request->query['areaname'].'%',
						'`Restaurant`.`access_station` LIKE '		=> '%'.$this->request->query['areaname'].'%',
						'`Restaurant`.`category_name_s` LIKE '      => '%'.$this->request->query['areaname'].'%',
						'`Restaurant`.`category_name_l` LIKE '      => '%'.$this->request->query['areaname'].'%',
						'`Restaurant`.`address` LIKE '       		=> '%'.$this->request->query['areaname'].'%',
						'`Movie`.`user_id` IN '        		 		=> $user_id_array
					),
					'Movie.del_flg' => 0
				);
	        ...
	        $Tagid = $this->TagRelation->find('all',array(
	        	'conditions'=>
	        		array( '`Tag`.`name` LIKE ' => '%'.$this->request->query['areaname'].'%'
	        			),
	        	'fields' => array('movie_id')
	        ));
